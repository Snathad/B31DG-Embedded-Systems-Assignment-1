...
//Aduino sequence version 6: simple with switch one input (enable/disable pulses) delays not quite right//
...
//if(s1==1 && flag1==1){
  ...
  if (ps1s==HIGH){  //if switch 1 is still high set s1 marker to high
    ...
    if (digitalRead(push_switch2)==HIGH){  //if switch 1 is still high set s1 marker to high
    s2=1;
  }
    ...
    delay2(b);// wait the length of B between individual pulses
     ...
     if (digitalRead(push_switch1)==HIGH){
      s1=1;
       ...
       if (digitalRead(push_switch2)==HIGH){
      s2=1;
       } 
      } 
